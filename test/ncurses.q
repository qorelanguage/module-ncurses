...
class MyPanel inherits Panel {
    constructor($x1, $y1, $x2, $y2): Panel($x1, $y1, $x2, $y2) {}
    nothing myBox($y, $x, $lines, $cols) {
        if ($y + $lines >= $.getLines())
            $lines = $.getLines() - $y - 1;
    ...
    nothing decorate($title, $color1, $color2) {
...
nothing sub ncurses_test_status(ncurses_test $obj) {
    background $obj.status();
}
nothing sub ncurses_test_thread_test(ncurses_test $obj) {
    background $obj.thread_test();
}
...
class ncurses_test {
    ...
    private menu(){
        ...
        while ((my $c = $x.getch()) != ord('q') && $c != ord('Q')) {
            ...
            case 't':
                ...
                background ncurses_test_thread_test($self);
                break;
    ...
    private move_test() {
        ...
        while ((my $c = $y.getch()) != ord("q")) {
            #$y.printw("y=%d, x=%d, c=%d, LEFT=%d, DOWN=%d\n", $y.getBegY(), $y.getBegX(), $c, KEY_LEFT, KEY_DOWN);
            my $rc;
            switch ($c) {
            case KEY_LEFT:
    ...
    constructor() {
        ...
        $.w = new MyPanel("xxX", getLines() - 1, getColumns(), 0, 0);
        if (1) return;
